...
const Registration = () => {
    ...
    const onSubmit = async data => {
        ...
        if (data.password === data.confirmPassword) {
            let url = 'http://localhost:5000/v1/user';
            fetch(url, {
                method: "POST",
                headers: {
    ...
    let signInError;
    if (error || gError) {
        signInError = <p className='text-red-500 mb-2'><small>{error?.message || gError?.message?.message}</small></p>
    }
    return (
        <div className="">
        <section className="registration-bg relative">
          <div className="">
            ...
            <div className="relative py-28">
              <h1 className="text-5xl text-white text-center mt-5">Registration</h1>
            </div>
          </div>
        </section>
        <section>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} action="">
                        <div className="grid grid-cols-2 gap-5">
                            ...
                            <div className="form-control">
                                ...
                                <input type="phone" className="input input-bordered w-full max-w-xs"
                                    {...register("phone", { 
                                        required: {
                                            value: true,
                                ...
                                <label class="label">
                                    {errors.phone?.type === 'required' && <span class="label-text-alt text-red-500">{errors.address.message}</span>}
                                </label>
                            </div>   
                            {/* address */}      
                            <div className="form-control">
